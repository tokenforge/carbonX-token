...
describe('CarbonX Vault Tests', () => {
    ...
    describe('we can stake tokens into vault', async () => {
        ...
        it('Removing the Carbon token from supported tokens would let any staking revert', async () => {
            await expect(vault.removeSupportedToken(token.address))
                .to.emit(vault, 'SupportedTokenRemoved')
                .withArgs(governance.address, token.address)
            
            expect(axelAsSigner.safeTransferFrom(axel.address, vault.address, tokenId, amount, '0x'))
                .to.be.revertedWithCustomError(vault, 'ErrTokenNotSupported')
                .withArgs(axel.address, token.address)
